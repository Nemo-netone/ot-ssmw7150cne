		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('discusshuodongxinxi','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('discusshuodongxinxi','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getDiscussList(page) {
				this.$http.get('discussdiscusshuodongxinxi/list', {params: {page, limit: this.pageSize, refid: this.detail.id,sort: 'istop',order: 'desc'}}).then(res => {
					if (res.data.code == 0) {
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discussdiscusshuodongxinxi/update',row).then(rs=>{
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						...
						this.$http.post('discussdiscusshuodongxinxi/add', this.form).then(rs2 => {
							if (rs2.data.code == 0) {
								this.form.content = '';
			...
			// 删除
			async delClick(){
				await this.$confirm('是否删除此活动信息评论表？') .then(_ => {
