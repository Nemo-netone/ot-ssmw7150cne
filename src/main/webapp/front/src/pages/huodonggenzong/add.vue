<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item"  label="活动状态" prop="huodongzhuangtai">
	...
	export default {
		...
		methods: {
			...
			init(type) {
				...
				this.huodongzhuangtaiOptions = "报名中,进行中,已结束".split(',')
			...
			// 多级联动参数
			info() {
				this.$http.get(`huodonggenzong/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`huodonggenzong/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
