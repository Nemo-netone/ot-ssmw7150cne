		...
		<el-form
			...
			<el-form-item class="add-item" label="结束时间" prop="jieshushijian">
				<el-input v-model="ruleForm.jieshushijian" 
					placeholder="结束时间" clearable :disabled=" false  ||ro.jieshushijian"></el-input>
			</el-form-item>
			<el-form-item class="add-item"  label="报名人数" prop="kecanyurenshu">
				<el-select v-model="ruleForm.kecanyurenshu" placeholder="请选择报名人数" :disabled=" false  ||ro.kecanyurenshu" >
					<el-option
						v-for="(item,index) in kecanyurenshuOptions"
						:key="index"
						:label="item"
						:value="item">
					</el-option>
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					this.ruleForm.kecanyurenshu = 0;
					this.ro.kecanyurenshu = false;
				}else if(type=='edit'){
					this.info()
				}
				// 获取用户信息
				...
				this.kecanyurenshuOptions = "1".split(',')
			...
			// 提交
			async onSubmit() {
				if(!this.ruleForm.kecanyurenshu){
					this.$message.error("报名人数不能为空");
					return
				}
				var obj = JSON.parse(localStorage.getItem('crossObj'));
				var table = localStorage.getItem('crossTable');
				obj.kecanyurenshu = obj.kecanyurenshu - this.ruleForm.kecanyurenshu
				if(obj.kecanyurenshu<0){
					...
					if(valid) {
						if(this.type=='cross'){
							...
							if(crossrefid && crossuserid) {
								...
								await this.$http.get('baomingxinxi/list', {
									params: params
								}).then(res => {
						...
						obj.kecanyurenshu = Number((parseFloat(obj.kecanyurenshu) - parseFloat(this.ruleForm.kecanyurenshu)).toFixed(2))

						await this.$http.post(table+`/update`,obj).then(res => {});
						await this.$http.post(`baomingxinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
